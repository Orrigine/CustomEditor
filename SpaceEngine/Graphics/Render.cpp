...
bool Render::Window::Initialize()
{
    ...
    // ShaderBase *mShaderBase = new ShaderBase(md3dDevice.Get(), 1, true);

    mShaderBase->BuildDescriptorHeaps();
    BuildConstantBuffers();
    mShaderBase->BuildRootSignature();
    mShaderBase->BuildShadersAndInputLayout();
    mShaderBase->BuildPSO();
...
void Render::Window::Draw(const GameTimer &gt)
{
    ...
    ID3D12DescriptorHeap *descriptorHeaps[] = {mShaderBase->mCbvHeap.Get()};
    mCommandList->SetDescriptorHeaps(_countof(descriptorHeaps), descriptorHeaps);

    mCommandList->SetGraphicsRootSignature(mShaderBase->mRootSignature.Get());
    mCommandList->SetPipelineState(mShaderBase->mPSO.Get());
    ...
    mCommandList->SetGraphicsRootDescriptorTable(0, mShaderBase->mCbvHeap->GetGPUDescriptorHandleForHeapStart());
...
void Render::Window::BuildConstantBuffers()
{
    ...
    md3dDevice->CreateConstantBufferView(&cbvDesc, mShaderBase->mCbvHeap->GetCPUDescriptorHandleForHeapStart());
}
